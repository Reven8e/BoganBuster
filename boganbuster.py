...
def check(buster):
    global timeout, Type, checked, f, timeout, TARGET
    headers = {'User-Agent': ''}
    user_agent_list = [
...
try:
    with open(f"{FILE}.txt", 'r', encoding="utf-8", errors='ignore') as f:
        ...
        for _ in range(thr):
            if checked < LEN:
                for buster in busts:
                        p = multiprocessing.Process(target=check, args=[buster])
                        p.start()
                        checked += 1
                        processes.append(p)
    ...
    print(f'{Fore.RED}[CONSOLE] Proxy list not found.')
    sys.exit()
